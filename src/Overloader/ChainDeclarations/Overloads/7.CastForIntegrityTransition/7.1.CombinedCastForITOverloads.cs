...
public sealed class CombinedCastForITOverloads : IChainMember
{
	ChainAction IChainMember.Execute(GeneratorProperties props, SyntaxNode syntaxNode)
	{
		...
		Span<int> maxTransitionsCount = stackalloc int[props.Store.FormattersIntegrityCount];
		ushort countOfCombineWith = 0;
		for (int index = 0, formatterIndex = 0; index < parameters.Count; index++)
		{
			var parameter = parameters[index];
			var mappedParam = props.Store.OverloadMap![index];
			if (mappedParam.ParameterAction is not ParameterAction.FormatterIntegrityReplacement)
				continue;
		...
		maxTransitionsCount = maxTransitionsCount.Slice(0, props.Store.FormattersIntegrityCount - countOfCombineWith);
		...
		for (;;)
		{
			...
			parameterBuilder
				.AppendChainMemberNameComment(nameof(CombinedCastForITOverloads))
				...
				.WriteTransitionOverload(
					...
					parameters,
					transitionIndexes,
					(index, formatter) => formatter.CastsForIntegrity[index],
					true);
